...
const Order = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8099/api/order?page=0&size=4")
      .then((response) => {
  ...
  const handlePageClick = (event: any, page: number) => {
    setCurrentPage(page);
    try {
      let apiUrl = `http://localhost:8099/api/order?page=${page - 1}&size=4`;
  ...
  const debouncedSearch = debounce(async (orderNumber: string, userNumber: string, fullName: string, etd: string, eta: string, status: string) => {
    try {
      let apiUrl = `http://localhost:8099/api/order?page=${currentPage - 1}&size=4`;
