...
import { GetUserResponse, IncidentInvoiceResponse } from "../../model/invoice";
...
function IncidentInvoice() {
  ...
  const importData = async () => {
    if (file) {
      ...
      await httpClient
        .post("/api/invoice/incident/upload", formData)
        .then((res) => {
  ...
  const downloadSample = async () => {
    await httpClient
      .get("api/blob/download", {
        params: {
          filename: "excel_sample/incident.xlsx",
        },
        responseType: "blob",
      })
      .then((res) => {
  ...
  const exportData = async () => {
    await httpClient
      .get("api/invoice/incident/download", { responseType: "arraybuffer" })
      .then((res) => {
        const file = new Blob([res.data], {
        ...
        let a = document.createElement("a");
        a.href = url;
        a.download = "hoadonthiethai_" + dayjs().format("YYYY-MM-DD") + ".xlsx";
        a.click();
        a.remove();
        handleClose();
      })
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () => fetchIncidentInvoiceData(page),
  });
  const clearFilter = () => {
  ...
  const fetchIncidentInvoiceData = async (page: number) =>
    await httpClient
            ...
            <table className="w-full table-auto ">
              <thead className="bg-secondary hover:cursor-default">
                <tr className="text-center dark:bg-meta-4 ">
                  ...
                  <th className="min-w-[80px] py-4 px-4 font-bold text-center text-black dark:text-white">
                    Thanh toán
                  </th>
              ...
              <tbody className="hover:cursor-default">
                {data && data.incidentInvoices?.length > 0 ? (
                  data.incidentInvoices.map((invoice, index) => (
                    <tr className="hover:bg-stroke text-center" key={index}>
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 text-black dark:border-strokedark">
                        {invoice.driver?.firstName +
                          " " +
                          invoice.driver?.lastName}
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 text-black dark:border-strokedark">
                        {invoice.driver?.userNumber}
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">
                          {invoice.cost ? invoice.cost.toLocaleString() : 0}
                        </p>
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div className="flex items-center space-x-3.5">
                          <button
                            onClick={() =>
                              navigate(
                                "/invoice/incident/" + invoice.incidentInvoiceId
                              )
                            }
...
export default IncidentInvoice;
