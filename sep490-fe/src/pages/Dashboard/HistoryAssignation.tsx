...
const HistoryAssignation = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8099/api/schedule?page=0&size=4`)
      .then((response) => {
  ...
  const handlePageClick = (event: any, page: number) => {
    setCurrentPage(page);
    try {
      let apiUrl = `http://localhost:8099/api/schedule?page=${page - 1}&size=4`;
  ...
  const debouncedSearch = debounce(
    async (
      ...
      try {
        let apiUrl = `http://localhost:8099/api/schedule?page=${
          currentPage - 1
        }&size=4`;
  ...
  return (
    <>
      <Breadcrumb pageName="Lịch sửa điều phối" />
