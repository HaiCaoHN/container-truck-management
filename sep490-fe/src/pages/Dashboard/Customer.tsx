...
const Customer = () => {
  ...
  const handleClose = async () => {
    ...
    try {
      let apiUrl = `http://localhost:8099/api/user?role=CUSTOMER&page=${currentPage - 1
        }&size=7`;
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8099/api/user?page=0&size=7&role=CUSTOMER")
      .then((response) => {
        setUsers(response.data.content);
        setTotalPages(response.data.totalPages);
      })
  ...
  const handleSearchPersonalId = async (event: SelectChangeEvent<string>) => {
    ...
    debouncedSearch(inputValue, firstName, userNumber, phone, email, status);
  };
  const handleSearchFirstName = async (event: SelectChangeEvent<string>) => {
    const inputValue = event.target.value;
    setFirstName(inputValue);
    debouncedSearch(personalId, inputValue, userNumber, phone, email, status);
  };
  const handleSearchuserNumber = async (event: SelectChangeEvent<string>) => {
    const inputValue = event.target.value;
    setuserNumber(inputValue);
    debouncedSearch(personalId, firstName, inputValue, phone, email, status);
  };
  const handleSearchPhone = async (event: SelectChangeEvent<string>) => {
    const inputValue = event.target.value;
    setPhone(inputValue);
    debouncedSearch(personalId, firstName, userNumber, inputValue, email, status);
  };
  const handleSearchEmail = async (event: SelectChangeEvent<string>) => {
    const inputValue = event.target.value;
    setEmail(inputValue);
    debouncedSearch(personalId, firstName, userNumber, phone, inputValue, status);
  };
  const handleStatusChange = (e: SelectChangeEvent<string>) => {
    const inputValue = e.target.value;
    setStatus(e.target.value);

    debouncedSearch(personalId, firstName, userNumber, phone, email, inputValue);
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-10">
        <div className="rounded-sm border border-stroke bg-white px-3 pt-4 pb-2 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-4 xl:pb-1">
          <div className="max-w-full overflow-x-auto">
            <table className="w-full table-auto">
              <thead>
                <tr className="bg-secondary hover:cursor-default">
                  <th className="min-w-[80px] py-1 px-1 font-bold text-black dark:text-white xl:pl-3">
                    CCCD
                  </th>
                ...
                <tr className="bg-secondary hover:cursor-default">
                  <th className="min-w-[80px] py-2 px-2 font-normal  text-black dark:text-white xl:pl-3">
                    <input
                      type="text"
                  ...
                  <th className="min-w-[80px] py-2 px-2 font-normal text-black dark:text-white">
                    <FormControl sx={{ m: 0, minWidth: 150 }} size="small">
                      <InputLabel id="label_filter_status">Status</InputLabel>
                      <Select
                        className="bg-white "
                        name="isActive"
                  ...
                  <th className="py-2 px-2 font-normal text-black dark:text-white">
                ...
                {users && users.length > 0 ? (
                  users.map((user) => (
                    <tr className="text-center" key={user.userId}>
                      <td className="border-b border-[#eee] hover:cursor-pointer  py-5 px-4 pl-9 dark:border-strokedark xl:pl-3">
